<!-- Google Tag Manager (noscript) -->
<noscript><iframe src="https://www.googletagmanager.com/ns.html?id=GTM-PQ2DJDQ" height="0" width="0"
    style="display:none;visibility:hidden"></iframe></noscript>
<!-- End Google Tag Manager (noscript) -->
<header>
  <header class="header">
    <div class="header__container">
      <!-- LOGO -->
      <a href="#" class="logo">
        <span class="logo__text">
          Bronquisol
        </span>
      </a>
      <!-- END LOGO -->
      <!-- NAV -->
      <nav class="nav">
        <ul class="nav__list">
          <!-- NAV ITEM -->
          <li class="nav__list__item">
            <a href="#home" class="nav__list__link anchor">
              Home
            </a>
          </li>
          <!-- END NAV ITEM -->
          <!-- NAV ITEM -->
          <li class="nav__list__item">
            <a href="#products" class="nav__list__link anchor">
              Producto
            </a>
          </li>
          <!-- END NAV ITEM -->
          <!-- NAV ITEM -->
          <li class="nav__list__item">
            <a href="#" class="nav__list__link">
              Legal
            </a>
          </li>
          <!-- END NAV ITEM -->
          <!-- NAV ITEM -->
          <li class="nav__list__item">
            <a href="https://larebajavirtual.com/catalogo/buscar?subMenuCategory=on&busqueda=bronquisol"
              class="nav__list__link">
              Compra
            </a>
          </li>
          <!-- END NAV ITEM -->
          <!-- NAV ITEM -->
          <li class="nav__list__item">
            <a href="#maps" class="nav__list__link anchor">
              <svg width="14" height="20" viewBox="0 0 14 20" fill="none" xmlns="http://www.w3.org/2000/svg"
                class="nav__list__icon">
                <path
                  d="M7 0C3.13 0 0 3.13 0 7C0 12.25 7 20 7 20C7 20 14 12.25 14 7C14 3.13 10.87 0 7 0ZM7 9.5C5.62 9.5 4.5 8.38 4.5 7C4.5 5.62 5.62 4.5 7 4.5C8.38 4.5 9.5 5.62 9.5 7C9.5 8.38 8.38 9.5 7 9.5Z"
                  fill="#00A3E6" />
              </svg>
            </a>
          </li>
          <!-- END NAV ITEM -->
        </ul>
      </nav>
      <!-- END NAV -->
      <!-- HAMBURGER -->
      <a href="#" class="hamburgerBtn">
        <span class="hamburgerBtn__line"></span>
        <span class="hamburgerBtn__line"></span>
        <span class="hamburgerBtn__line"></span>
      </a>
      <!-- END HAMBURGER -->
    </div>
  </header>
  <?php // 👉👉 get_search_form(); ?>

  <!-- ↓↓↓ HEADER CONTENT HERE ↓↓↓ -->
</header>